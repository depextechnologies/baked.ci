...
export const MobileRefer = () => {
  ...
  const shareVia = (channel) => {
    ...
    const msg = `Join me on BAKĒD — use my code ${r?.referral_code} to sign up. ${url}`;
    ...
    else if (channel === "email") window.location.href = `mailto:?subject=${encodeURIComponent("Join me on BAKĒD")}&body=${encodeURIComponent(msg)}`;
    else if (channel === "native" && navigator.share) navigator.share({ title: "Join me on BAKĒD", text: msg, url }).catch(() => {});
  ...
  return (
    <div className="pb-24">
      <div className="px-4 pt-2 pb-3 flex items-center gap-2">
        <button data-testid="m-ref-back" onClick={() => nav("/profile")} className="w-9 h-9 rounded-full bg-secondary flex items-center justify-center"><ArrowLeft size={16} /></button>
        <div className="flex-1 min-w-0"><div className="text-base font-bold">Refer & Earn</div><div className="text-[11px] text-muted-foreground">Invite friends. They join. You earn.</div></div>
      ...
      <div className="px-4">
        <div className="relative rounded-3xl overflow-hidden p-6 text-center" style={{ background: "linear-gradient(135deg, #0a1a3a 0%, #0a1200 50%, #1a0a3a 100%)" }}>
          ...
          <div className="relative">
            ...
            <div className="text-lg font-bold text-white leading-tight">Earn {formatMoney(r.reward_per_referral, r.currency, r.currency === "XOF" ? "CFA" : "£")} for every successful referral!</div>
            <div className="text-[11px] text-white/60 mt-2">Referrals are tracked for {r.validity_days} days. Rewards activate when the wallet launches.</div>
          </div>
        </div>
      </div>
      ...
      <div className="px-4 mt-4">
        <div className="baked-card bg-card border border-border p-4">
          <div className="text-[10px] uppercase tracking-widest text-muted-foreground">Your referral code</div>
          <div className="mt-2 flex items-center gap-3">
            ...
            <button data-testid="m-ref-copy-code" onClick={() => copy(r.referral_code, "Code copied")} className="w-11 h-11 rounded-2xl flex items-center justify-center bg-secondary motion-fast active:scale-95" aria-label="Copy code"><Copy size={17} /></button>
          </div>
          <div className="mt-3 pt-3 border-t border-border">
            <div className="text-[10px] uppercase tracking-widest text-muted-foreground">Referral link</div>
            <div className="mt-2 flex items-center gap-2">
              ...
              <button data-testid="m-ref-copy-link" onClick={() => copy(r.referral_link, "Link copied")} className="text-xs font-bold px-3 h-9 rounded-lg motion-fast active:scale-95 text-black" style={{ backgroundColor: "#77BC1F" }}>Copy</button>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Share via */}
      <div className="px-4 mt-4">
        <div className="text-sm font-bold mb-2">Share via</div>
        <div className="grid grid-cols-4 gap-2">
          {[
            { code: "whatsapp", icon: MessageCircle, label: "WhatsApp", tone: "#25D366" },
            { code: "sms", icon: Smartphone, label: "SMS", tone: "#1D9BF0" },
            { code: "email", icon: Mail, label: "Email", tone: "#FCC44C" },
            { code: "native", icon: Share2, label: "More", tone: "#A659FF" },
      ...
      {/* Stats */}
      <div className="px-4 mt-4">
        <div className="text-sm font-bold mb-2">Your referral stats</div>
        <div className="grid grid-cols-3 gap-2">
          <div className="baked-card bg-card border border-border p-3 text-center">
            ...
            <div className="text-[10px] uppercase tracking-widest text-muted-foreground mt-0.5">Friends joined</div>
          </div>
          <div className="baked-card bg-card border border-border p-3 text-center">
            <div className="text-2xl font-bold" style={{ color: "#FCC44C" }} data-testid="m-ref-stat-pending">{r.pending}</div>
            <div className="text-[10px] uppercase tracking-widest text-muted-foreground mt-0.5">Pending</div>
          </div>
          <div className="baked-card bg-card border border-border p-3 text-center">
            <div className="text-2xl font-bold" style={{ color: "#77BC1F" }} data-testid="m-ref-stat-earned">{formatMoney(r.total_earned, r.currency, r.currency === "XOF" ? "CFA" : "£")}</div>
            <div className="text-[10px] uppercase tracking-widest text-muted-foreground mt-0.5">Total earned</div>
          </div>
        </div>
        {r.friends_joined === 0 && (
          <div className="text-[11px] text-muted-foreground text-center mt-2">Track pending — invites are counted for {r.validity_days} days. Share your code to get started.</div>
        )}
      </div>
      ...
      <div className="px-4 mt-5">
        ...
        <div className="baked-card bg-card border border-border p-4 space-y-4">
          {[
            { icon: Users2, title: "Invite friends", sub: "Share your code or link", tone: "#1D9BF0" },
            { icon: Sparkles, title: "They join", sub: "Your friend signs up & places an order", tone: "#FCC44C" },
            { icon: TrendingUp, title: "You earn", sub: `You both get ${formatMoney(r.reward_per_referral, r.currency, r.currency === "XOF" ? "CFA" : "£")} once the wallet launches`, tone: "#77BC1F" },
