        ...
        return (
          <li key={step.code} className="flex gap-3 relative pb-4 last:pb-0">
            ...
            <div className="flex-1 pt-0.5">
              <div className={`text-sm font-semibold ${upcoming ? "text-muted-foreground" : ""}`}>{step.label}</div>
              <div className="text-[10px] text-muted-foreground">{step.at ? new Date(step.at).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) : (now ? "In progress…" : "Pending")}</div>
            </div>
          </li>
        );
      })}
